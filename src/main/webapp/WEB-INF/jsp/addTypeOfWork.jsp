    ...
    <form method="post" action="deletePosition" >
        ...
        <table border="1" >
            ...
            <c:forEach  items="${typesOfWork}" var ="typeOfWork">
                <tr>
                    <td >${typeOfWork.idTypeOfWork}</td>
                    <td>${typeOfWork.name}</td>
                    <td><a href="/tables/typeOfWork?updId=${typeOfWork.idTypeOfWork}&flag=${true}">Редактирвоать</a></td>
                    <td><a href="/tables/delete/typeOfWork?updId=${beanUp.idTypeOfWork}&delId=${typeOfWork.idTypeOfWork}&flag=<%=flag==true?"true":"false"%>">Удалить</a></td>
                </tr>
            </c:forEach>
        </table>
    </form>
